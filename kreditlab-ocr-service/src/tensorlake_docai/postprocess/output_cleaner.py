...
# Modified fromhttps://github.com/rednote-hilab/dots.mocr/blob/master/dots_ocr/utils/output_cleaner.py
# Added Extract Outermost JSON Array and plain text ocr output detection from original parsing code
# Improved the cleaning code to handle the last meaningful content and trailing empty/incomplete table elements
...
class OutputCleaner:
    ...
    def clean_string_data(self, data_str: str, case_id: int) -> CleanedData:
        ...
        try:
            # Step 0a: Extract outermost JSON array from explanatory text
            data_str, json_extracted = self._extract_outermost_json_array(data_str)
            operations["json_extracted"] = json_extracted

            # Step 0b: Check for plain text OCR output (no JSON structure at all)
            if self._is_plain_text_output(data_str):
                print("  ✓ Detected plain text OCR output (no JSON structure)")
                operations["plain_text_detected"] = True
                # Return empty result for plain text (no structured layout data)
                return CleanedData(
                    case_id=case_id,
                    original_type="str",
            ...
            data_str, duplicate_removes = self._remove_duplicate_complete_dicts_preserve_order(
                data_str
            )
    ...
    def _extract_outermost_json_array(self, text: str) -> Tuple[str, bool]:
        """Extract the outermost JSON array from text that may contain explanatory content"""
        ...
        if (
            first_bracket_idx != -1
            and last_bracket_idx != -1
            and last_bracket_idx > first_bracket_idx
        ):
            potential_json = text[first_bracket_idx : last_bracket_idx + 1]

            # Try to validate it's parseable JSON
            try:
                json.loads(potential_json)
                print(
                    f"    ✓ Extracted outermost JSON array from explanatory text (positions {first_bracket_idx}:{last_bracket_idx})"
                )
                return potential_json, True
            except json.JSONDecodeError:
                # If it doesn't parse, continue with original text
                # The rest of the cleaning pipeline will try to fix it
                pass

        return text, False

    def _is_plain_text_output(self, text: str) -> bool:
        """Check if this is plain text OCR output with no JSON structure"""

        # If there are no JSON structural characters, it's plain text
    ...
    def _clean_trailing_empty_table_cells(self, text: str) -> str:
        """Remove trailing empty table cells and rows from text content"""
        if "<table>" not in text.lower():
            return text

        original_len = len(text)

        # Strategy 1: Find the last cell/element with actual text content (not just empty tags)
        # Match any text between tags that's not whitespace/empty
        last_meaningful_pos = -1

        # Look for the last occurrence of actual text content in cells or other elements
        # This matches: <tag>actual_content</tag> where actual_content is not empty
        for match in re.finditer(r"<(td|th|caption|h\d)[^>]*>([^<]+)</\1>", text, re.IGNORECASE):
            content = match.group(2).strip()
            # Check if content has substance (not just whitespace, dots, dashes, etc.)
            if content and len(content) > 0 and not re.match(r"^[\s\-_.]*$", content):
                last_meaningful_pos = match.end()

        # If no meaningful content found in cells, look for table structural tags
        if last_meaningful_pos == -1:
            # Try to find last </thead> or </tbody> with content
            for match in re.finditer(r"</(thead|tbody)>", text, re.IGNORECASE):
                last_meaningful_pos = match.end()

        # If we found where meaningful content ends, truncate there
        if last_meaningful_pos > 0 and last_meaningful_pos < len(text):
            # Add a safety margin - look a bit further to catch closing tags
            # Find the next </tr> or </table> after the last meaningful content
            next_closing = text.find("</tr>", last_meaningful_pos)
            if next_closing != -1 and next_closing < last_meaningful_pos + 1000:
                last_meaningful_pos = next_closing + 5  # Include the </tr>

            text = text[:last_meaningful_pos]
            print(
                f"    ✂️ Truncated text after last meaningful content at position {last_meaningful_pos}"
            )

        # Now aggressively remove trailing patterns of empty cells and rows
        # Keep removing until no more matches
        max_iterations = 10
        for i in range(max_iterations):
            before_len = len(text)

            # Remove any trailing incomplete tags first
            text = re.sub(r"<[^>]*$", "", text, flags=re.IGNORECASE)

            # Remove trailing empty rows: <tr><td></td><td></td>...</tr> or <tr></tr>
            text = re.sub(
                r"(<tr[^>]*>\s*(<td[^>]*>\s*</td>\s*)*\s*</tr>\s*)+$", "", text, flags=re.IGNORECASE
            )

            # Remove trailing empty cells (not in complete rows)
            text = re.sub(r"(<td[^>]*>\s*</td>\s*)+$", "", text, flags=re.IGNORECASE)

            # Remove trailing incomplete rows (opening tr but no closing)
            text = re.sub(r"<tr[^>]*>\s*(<td[^>]*>\s*</td>\s*)*$", "", text, flags=re.IGNORECASE)

            # Remove trailing incomplete cells
            text = re.sub(r"<td[^>]*>?\s*$", "", text, flags=re.IGNORECASE)

            # Remove other incomplete opening tags
            text = re.sub(r"<(?:table|thead|tbody|th|caption)[^>]*$", "", text, flags=re.IGNORECASE)
        ...
        if chars_removed > 0:
            print(
                f"    🧹 Cleaned {chars_removed:,} chars of trailing empty/incomplete table elements"
            )

        return text

    def _truncate_last_incomplete_element(self, text: str) -> Tuple[str, bool]:
        """Truncates the last incomplete element, but tries to preserve valid bbox/category"""

        # For very long text (>50k) or text not ending with ']', check if last element needs truncation
        ...
        if needs_truncation:
            ...
            if last_bbox_pos > 0:
                # Try to salvage the last dict if it has valid bbox and category
                last_dict_str = text[last_bbox_pos:]

                # Check if we can extract valid bbox and category from the last dict
                bbox_match = re.search(r'"bbox"\s*:\s*\[([^\]]+)\]', last_dict_str)
                category_match = re.search(r'"category"\s*:\s*"([^"]+)"', last_dict_str)

                if bbox_match and category_match:
                    try:
                        # Try to parse the bbox coordinates
                        bbox_coords = [float(x.strip()) for x in bbox_match.group(1).split(",")]

                        if len(bbox_coords) == 4:
                            # Last dict has valid bbox and category, try to preserve it
                            print(
                                "    ✂️ Last element has valid bbox and category, attempting to preserve it"
                            )

                            # Try to extract text field, truncate if incomplete
                            text_match = re.search(r'"text"\s*:\s*"', last_dict_str)
                            if text_match:
                                text_start = text_match.end()
                                # Find a reasonable truncation point in the text (up to 50k chars max)
                                text_content = last_dict_str[text_start : text_start + 50000]

                                # Debug: show a sample of what we're about to clean
                                if len(text_content) > 1000:
                                    print(
                                        f"    📝 Text sample (last 200 chars before cleaning): ...{text_content[-200:]}"
                                    )

                                # Clean up trailing empty table cells from the text content
                                text_content = self._clean_trailing_empty_table_cells(text_content)

                                # If after cleaning there's substantial content, keep it
                                # Otherwise, keep bbox and category only
                                if len(text_content.strip()) > 10:
                                    # Build a valid dict with cleaned truncated text
                                    ...
                                    # Reconstruct: everything before last bbox + salvaged dict
                                    truncated_text = text[:last_bbox_pos].rstrip()
                                    if truncated_text.endswith(","):
                                        truncated_text = truncated_text[:-1]

                                    # Add the salvaged dict
                                    truncated_text += ", " + json.dumps(
                                        salvaged_dict, ensure_ascii=False
                                    )

                                    print(
                                        f"    ✅ Preserved last element with bbox {bbox_coords} and cleaned text ({len(text_content)} chars)"
                                    )
                                    return truncated_text, True
                                else:
                                    # Text is empty or too short after cleaning, keep only bbox and category
                                    category = category_match.group(1)
                                    salvaged_dict = {"bbox": bbox_coords, "category": category}
                                    ...
                                    print(
                                        f"    ✅ Preserved last element with bbox {bbox_coords} (text was empty after cleaning)"
                                    )
                                    return truncated_text, True
                            else:
                                # No text field, keep bbox and category only
                                category = category_match.group(1)
                                salvaged_dict = {"bbox": bbox_coords, "category": category}
                                ...
                                print(
                                    f"    ✅ Preserved last element with bbox {bbox_coords} (no text field)"
                                )
                                return truncated_text, True

                    except (ValueError, json.JSONDecodeError) as e:
                        print(f"    ⚠️ Failed to salvage last dict: {e}")
                        pass

                # If we can't salvage it, truncate before the last bbox entirely
                truncated_text = text[:last_bbox_pos].rstrip()
                ...
                print(
                    f"    ✂️ Truncated the last incomplete element (couldn't salvage), length reduced from {len(text):,} to {len(truncated_text):,}"
                )
                return truncated_text, True
    ...
    def _remove_duplicate_complete_dicts_preserve_order(self, text: str) -> Tuple[str, int]:
        ...
        if total_duplicates > 0:
            ...
            print("    ✅ No duplicate dict objects found")
            return text, 0
    ...
    def _generate_cleaning_report(self, output_dir: str):
        ...
        print("  📋 Cleaning report: cleaning_report.txt")
